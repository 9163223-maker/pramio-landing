...
(() => {
  ...
  const draw=(ms,once=false)=>{
    ...
    ctx.restore();if(!once)raf=requestAnimationFrame(draw);
  };
  ...
  resize();
  if(matchMedia('(prefers-reduced-motion: reduce)').matches){draw(0,true);}else raf=requestAnimationFrame(draw);
})();
